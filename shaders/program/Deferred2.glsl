...
vec4 TemporalLightAccumulation() {
	if (all(lessThan(screenCoord, vec2(0.5)))) {
		...
		// =============================================================================
		// ВАНИЛЬНАЯ ЛОГИКА ПЕЩЕР - Читаем skylight из gbuffer
		// =============================================================================
		vec4 gbuffer7 = texelFetch(colortex7, texel, 0);
		float skyLight = gbuffer7.g; // Sky lightmap из gbuffer
		bool hasSkyAccess = skyLight > 0.05; // Доступ к небу?
		...
		// =============================================================================
		// GI только если есть доступ к небу (ванильная логика)
		// =============================================================================
		#ifdef GI_ENABLED
			if (hasSkyAccess) {
